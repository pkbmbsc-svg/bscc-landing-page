...
import { X, Upload, Send, Sparkles, Check, AlertCircle, Image as ImageIcon, Share2, Globe, Lock, KeyRound, ShieldCheck } from 'lucide-react';
...
const DEFAULT_ADMIN_PIN = 'adminbscc'; // Default Admin Web PIN/Password
...
export const AdminGalleryUploadModal: React.FC<AdminGalleryUploadModalProps> = ({
  ...
  const handleVerifyAdmin = (e: React.FormEvent) => {
    e.preventDefault();
    if (inputPin.trim().toLowerCase() === DEFAULT_ADMIN_PIN || inputPin.trim() === '123456') {
      setIsAuthenticated(true);
      setAuthError('');
      showToast('Akses Admin Web diverifikasi. Selamat datang, Pengelola BSCC!');
    } else {
      setAuthError('PIN / Password Admin Web salah. Silakan coba lagi.');
    }
  };
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/85 backdrop-blur-md animate-fade-in overflow-y-auto">
      <div className="bg-slate-900 border border-slate-700/80 rounded-2xl w-full max-w-2xl overflow-hidden shadow-2xl my-8">
          ...
          <div className="flex items-center space-x-3">
            ...
            <div>
              <h3 className="text-lg font-bold text-white flex items-center space-x-2">
                <span>Panel Posting Galeri</span>
              ...
              <p className="text-xs text-slate-400">
                Aktivitas publikasi dan integrasi auto-post terbatas hanya untuk Admin Web PKBM Berkah Sadaya
              </p>
            </div>
          </div>
          <button
        ...
        {!isAuthenticated ? (
          <div className="p-8 text-center max-w-md mx-auto space-y-6">
            ...
            <div>
              <h4 className="text-xl font-extrabold text-white mb-2">
                Otentikasi Admin Web PKBM
              </h4>
              <p className="text-xs text-slate-400 leading-relaxed">
                Aktivitas posting kegiatan dan auto-publish media sosial memerlukan verifikasi hak akses Admin Web.
              </p>
            </div>
            ...
            <form onSubmit={handleVerifyAdmin} className="space-y-4 text-left">
              <div>
                <label className="block text-xs font-semibold text-slate-300 uppercase tracking-wider mb-2">
                  Masukkan Password / PIN Admin Web
                </label>
                <input
                  type="password"
                  required
                  ...
                  className="w-full bg-slate-800 border border-slate-700 rounded-xl px-4 py-3 text-sm text-white placeholder-slate-500 focus:outline-none focus:border-amber-500 transition-colors text-center tracking-widest font-mono"
                  autoFocus
                />
                {authError && (
                  <p className="text-xs text-rose-400 mt-2 flex items-center space-x-1 font-medium">
                    <AlertCircle className="w-3.5 h-3.5 shrink-0" />
                    <span>{authError}</span>
                  </p>
                )}
                <p className="text-[11px] text-slate-500 mt-2 text-center">
                  Password Admin Demo: <code className="text-amber-400 font-mono">adminbscc</code>
                </p>
              </div>
              ...
              <div className="flex space-x-3 pt-2">
                ...
                <button
                  type="submit"
                  className="w-1/2 py-2.5 bg-amber-500 hover:bg-amber-400 text-slate-950 font-extrabold text-xs rounded-xl shadow-lg transition-all flex items-center justify-center space-x-2"
                >
          ...
          /* STEP 2: POSTING & PUBLISHING FORM FOR ADMIN WEB */
          <form onSubmit={handleSubmit} className="p-6 space-y-5 max-h-[75vh] overflow-y-auto">
            <div className="p-3 bg-emerald-500/10 border border-emerald-500/30 rounded-xl flex items-center justify-between text-xs text-emerald-400 font-medium">
              <span className="flex items-center space-x-2">
                <ShieldCheck className="w-4 h-4 text-emerald-400 shrink-0" />
                <span>Terverifikasi sebagai <strong>Admin Web PKBM Berkah Sadaya</strong></span>
              </span>
              <button
                type="button"
